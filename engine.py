...
class Engine:
    ...
    def __init__(self, game):
        ...
        self._resize_tileset(self.game.game_map)
    ...
    def __load_textures(self):
        self.tex = {
            "Wall": pygame.transform.scale(
                pygame.image.load("textures/Wall_brick.png"),
                (self.block_size[0], self.block_size[1])),
            "Floor": pygame.transform.scale(
                pygame.image.load("textures/Weg1.png"),
                (self.block_size[0], self.block_size[1])
            )
        }

    def _resize_tileset(self, game_map):
        """
        Changes the tileset size according to the map size
        """

        ssize = (19., 19.)
        msize = game_map.map.shape
    ...
    def draw_map(self, game_map):
        ...
        for xgrid, row in enumerate(game_map):
            for ygrid, content in enumerate(row):
                ...
                if content == 1:
                    ...
                    if not self.game.opt["LowGraphic"]:
    ...
    def draw_pellets(self, pellets):
        for p in pellets:
            ...
            if not self.game.opt["LowGraphic"]:
                pygame.draw.circle(self.screen, py_black, (
                    int(xs + self.block_size[0] / 2.) + 1, int(ys + self.block_size[1] / 2.) + 1), self.pellet_size)
            pygame.draw.circle(self.screen, py_white,
                               (int(xs + self.block_size[0] / 2.), int(ys + self.block_size[1] / 2.)), self.pellet_size)

    def draw_bot(self, bots, color):
        for bot in bots:
            # first draw the trail
            for lindex, lp in enumerate(bot.get_last_position()):
                xs, ys = self.convert_grid_to_screen_coord((lp[0], lp[1]))
                pygame.draw.circle(self.screen, color,
                                   (int(xs + self.block_size[0] / 2.), int(ys + self.block_size[1] / 2.)),
                                   int(0.5 * self.bot_size * (lindex + 1) / 11. + self.trace_width), self.trace_width)

            # then draw the bot
            xs, ys = self.convert_grid_to_screen_coord(bot.get_position())

            if not self.game.opt["LowGraphic"]:
                pygame.draw.circle(self.screen, py_black, (int(xs + self.block_size[0] / 2. + 2),
                                                           int(ys + self.block_size[1] / 2.) + 2), self.bot_size)
            pygame.draw.circle(self.screen, color, (int(xs + self.block_size[0] / 2.),
                                                    int(ys + self.block_size[1] / 2.)), self.bot_size)
    ...
    def draw_UI(self, teams):
        ...
        text = self.fontS.render(teams[0].name + " " + "%03d" % teams[0].score, True, teams[0].color)
        warning = self.fontW.render("Movement Warnings " + "%03d" % teams[0].move_warns, True, teams[0].color)
        warning2 = self.fontW.render("Code Warnings " + "%03d" % teams[0].code_warns, True, teams[0].color)
        warning3 = self.fontW.render("TimeOut Warnings " + "%03d" % teams[0].timeout_warns, True, teams[0].color)
        kills = self.fontW.render("Kills " + "%03d" % teams[0].kills, True, teams[0].color)
        ...
        self.screen.blit(warning,
                         ((self.res[0] - 200) / 2 - 10 - warning.get_width(), self.res[1] - self.UI_height * 0.6))
        self.screen.blit(warning2,
                         ((self.res[0] - 200) / 2 - 10 - warning2.get_width(), self.res[1] - self.UI_height * 0.55))
        self.screen.blit(warning3,
                         ((self.res[0] - 200) / 2 - 10 - warning3.get_width(), self.res[1] - self.UI_height * 0.5))

        text = self.fontS.render("%03d " % teams[1].score + teams[1].name, True, teams[1].color)
        warning = self.fontW.render("%03d" % teams[1].move_warns + " Movement Warnings", True, teams[1].color)
        warning2 = self.fontW.render("%03d" % teams[1].code_warns + " Code Warnings", True, teams[1].color)
        warning3 = self.fontW.render("%03d" % teams[1].code_warns + " Timeout Warnings", True, teams[1].color)
        kills = self.fontW.render("%03d" % teams[1].kills + " Kills", True, teams[1].color)
